...
import newlib.Colors;
import newlib.Settings;
import newlib.Sounds;
import newlib.Textures;
import newlib.MainController;
	...
	public Tile() {
		...
		this.setOnMouseClicked(event -> {
			...
			if (event.getButton() == MouseButton.PRIMARY)
			{
				...
				if (newlib.Game.STATUS.get() == false) {

					// New game
					newlib.Game.getInstance().clear();
				 ...
				 */
				else if ((newlib.Game.GAME_MODE.equals(Settings.HUMAN_VS_AI) && 
							newlib.Game.CURRENT_PLAYER.getValue().equals(Settings.CIRCLE)) ||
							newlib.Game.GAME_MODE.equals(Settings.MULTIPLAYERS)
				) {
					...
					if (newlib.Game.CURRENT_PLAYER.get().equals(Settings.CROSS)) {
					...
					newlib.Game.HIT.set(newlib.Game.HIT.get() + 1);
					
					// And change to the cross
					newlib.Game.CURRENT_PLAYER.set(nextPlayer);
				}
			}
		});
	}
